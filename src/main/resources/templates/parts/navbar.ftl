...
<nav class="navbar navbar-expand-md navbar-dark">
    ...
    <div class="mx-auto order-0">
        <a class="navbar-brand mx-auto" href="/menu">Food Family</a>
        ...
        <ul  class="navbar-nav ml-auto">
				<li class="dropdown" id="currentShoppingCart" style="cursor: pointer;">
					...
					<div class="dropdown-menu shopping-cart-desc" style="text-align:center;">
						<span class="total-cost">₴${(CURRENT_SHOPPING_CART.totalCost)!0}</span><br> 
           ...
           <li class="pl-2 pt-1">
				<form action="/" method="post">
						<input type="hidden" name="_csrf" value="${_csrf.token}" />
     						<select class="custom-select flag-icon flag-icon-<@spring.message "text.language.image"/>" name="lang" onchange="submit()">
     				 			<option hidden selected><@spring.message "text.language"/></option>
     				 			<option value="en">English</option>
        			 	 		<option value="ru">Русский</option>
       				 	 		<option value="ua">Українська</option></span>
   				   	  		</select>
 		   		</form>
 		   	</li>
        </ul>
